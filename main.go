...
func init() {
	...
	if printVersion {
		_, _ = fmt.Fprintf(os.Stdout, "Air %s\n", VERSION)
		os.Exit(0)
	}
...
func nodeSync(idIndex uint32, w *WaitGroupWrapper) (err error) {
	...
	var (
		usersBefore, usersNow *[]UserInfo
		usersTraffic          *[]UserTraffic
		proxyClient           ProxyCommand
		panelClient           PanelCommand
		nodeBefore            *NodeInfo
	)

	nodeID := baseCfg.Panel.NodeIDs[idIndex]
	nodeBefore = new(NodeInfo)
	usersBefore = new([]UserInfo)
	usersNow = new([]UserInfo)
	...
	for {
		...
		if baseCfg.Proxy.SpeedLimitLevel != nil {
			// 限速匹配
			err = AddLevel(usersNow, baseCfg.Proxy.SpeedLimitLevel)
			if err != nil {
		...
		useRemove, userAdd, err := FindUserDiffer(usersBefore, usersNow)
		if err != nil {
