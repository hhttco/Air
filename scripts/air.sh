...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/hhttco/Air/refs/heads/main/scripts/install.sh) $version
    if [[ $? == 0 ]]; then
        echo -e "${green}更新完成，已自动重启 Air，请使用菜单页内的查看日志查看运行日志${plain}"
        exit
    fi
...
config() {
    cat /usr/local/etc/air/air.json
}

uninstall() {
    confirm "确定要卸载 Air 吗?" "n"
    if [[ $? != 0 ]]; then
        if [[ $# == 0 ]]; then
            show_menu
        fi
    ...
    systemctl stop air
    systemctl disable air
    rm /etc/systemd/system/air.service -f
...
start() {
    ...
    if [[ $? == 0 ]]; then
        echo ""
        echo -e "${green}Air 已运行，无需再次启动，如需重启请选择重启${plain}"
    else
        systemctl start air
        sleep 2
        check_status
        if [[ $? == 0 ]]; then
            echo -e "${green}Air 启动成功，请使用查看运行日志${plain}"
        else
            echo -e "${red}Air 可能启动失败，请稍后使用菜单查看日志信息${plain}"
        fi
    fi
...
stop() {
    ...
    check_status
    if [[ $? == 1 ]]; then
        echo -e "${green}Air 停止成功${plain}"
    else
        echo -e "${red}Air 停止失败，可能是因为停止时间超过了两秒，请稍后查看日志信息${plain}"
    fi
...
restart() {
    ...
    check_status
    if [[ $? == 0 ]]; then
        echo -e "${green}Air 重启成功，请使用菜单查看运行日志${plain}"
    else
        echo -e "${red}Air 可能启动失败，请稍后使用菜单查看日志信息${plain}"
    fi
    if [[ $# == 0 ]]; then
...
status() {
    systemctl status air --no-pager -l
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
}
...
enable() {
    ...
    if [[ $? == 0 ]]; then
        echo -e "${green}Air 设置开机自启成功${plain}"
    else
        echo -e "${red}Air 设置开机自启失败${plain}"
    fi
...
disable() {
    systemctl disable air
    if [[ $? == 0 ]]; then
        echo -e "${green}Air 取消开机自启成功${plain}"
    else
        echo -e "${red}Air 取消开机自启失败${plain}"
    fi
...
show_log() {
    journalctl -u air.service -e --no-pager -f
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
}
...
update_shell() {
    wget -O /usr/bin/air -N --no-check-certificate https://raw.githubusercontent.com/hhttco/Air/refs/heads/main/configs/air.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_status() {
    if [[ ! -f /etc/systemd/system/air.service ]]; then
        return 2
    fi
    temp=$(systemctl status air | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_uninstall() {
    ...
    if [[ $? != 2 ]]; then
        echo ""
        echo -e "${red}Air 已安装，请不要重复安装${plain}"
        if [[ $# == 0 ]]; then
            before_show_menu
        fi
        return 1
...
acme() {
  mkdir -p /usr/local/share/air/
  chmod -R 755 /usr/local/share/air
  cert_path="/usr/local/share/air/server.crt"
  key_path="/usr/local/share/air/server.key"
  rm /usr/local/share/air/server.key
  rm /usr/local/share/air/server.crt
...
get_latest_air_version() {
  # Get Xray latest release version number
  local tmp_file
  tmp_file="$(mktemp)"
  if ! curl -x "${PROXY}" -sS -H "Accept: application/vnd.github.v3+json" -o "$tmp_file" 'https://api.github.com/repos/hhttco/Air/releases/latest'; then
    "rm" "$tmp_file"
  ...
  if [[ -z "$RELEASE_LATEST" ]]; then
    if grep -q "API rate limit exceeded" "$tmp_file"; then
      ...
      echo "Welcome bug report:https://github.com/hhttco/Air/issues"
    fi
    "rm" "$tmp_file"
    exit 1
  fi
...
show_status() {
    ...
    case $? in
        0)
            echo -e "Air 状态: ${green}已运行${plain}"
            show_enable_status
            ;;
        1)
            echo -e "Air 状态: ${yellow}未运行${plain}"
            show_enable_status
            ;;
        2)
            echo -e "Air 状态: ${red}未安装${plain}"
    esac
}
...
open_air_config(){
  echo "如遇报错请安装vi"
  vi /usr/local/etc/air/air.json
}

show_usage() {
    echo "Air 管理脚本使用方法: "
    echo "------------------------------------------"
    echo "Air              - 显示管理菜单 (功能更多)"
    echo "Air start        - 启动 Air"
    echo "Air stop         - 停止 Air"
    echo "Air restart      - 重启 Air"
    echo "Air status       - 查看 Air 状态"
    echo "Air enable       - 设置 Air 开机自启"
    echo "Air disable      - 取消 Air 开机自启"
    echo "Air log          - 查看 Air 日志"
    echo "Air update x.x.x - 更新 Air 指定版本"
    echo "Air install      - 安装 Air"
    echo "Air uninstall    - 卸载 Air"
    echo "Air version      - 查看 Air 版本"
    echo "------------------------------------------"
}

show_menu() {
    echo -e "
  ${green}Air 后端管理脚本，${plain}${red}不适用于docker${plain}
--- https://github.com/hhttco/Air ---
  ${green}0.${plain} 退出脚本
————————————————
...
————————————————
  ...
  ${green}5.${plain} 停止 Air
  ${green}6.${plain} 重启 Air
  ${green}7.${plain} 查看 Air 状态
  ${green}8.${plain} 查看 Air 日志
————————————————
  ${green}9.${plain} 设置 Air 开机自启
 ${green}10.${plain} 取消 Air 开机自启
————————————————
 ...
 #后续更新可加入上方字符串中
    ...
    case "${num}" in
        ...
        11) install_bbr
        ;;
        12) check_install && show_air_version
        ;;
        13) check_install && update_xray && restart
        ;;
        14) check_install && identify_the_operating_system_and_architecture && get_latest_air_version && update_air && restart
        ;;
