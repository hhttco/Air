...
Installation_dependency() {
  ...
  mkdir /var/log/air
  chown -R nobody /var/log/air
}

download() {
  mkdir /usr/local/etc/air/
  air_url="https://github.com/hhttco/Air/releases/download/${VERSION}/Air-linux-${MACHINE}.zip"
  xray_json_url="https://raw.githubusercontent.com/hhttco/Air/refs/heads/main/configs/xray_config.json"
...
makeConfig() {
  ...
  cat >/usr/local/etc/air/air.json <<EOF
{
  "panel": {
    "type": "${panelType}",
    "url": "${pUrl}",
...
EOF
chmod 644 /usr/local/etc/air/air.json
}

createService() {
  service_file="https://raw.githubusercontent.com/hhttco/Air/refs/heads/main/configs/air.service"
  wget -N  -O /etc/systemd/system/air.service ${service_file}
  chmod 644 /etc/systemd/system/air.service
